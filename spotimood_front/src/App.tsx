    ...
    >
      {isLoading && !received ? (
        ...
        <>
          ...
          <><a className="playlist" href={link} target='_blank'>Open in Spotify</a><div className="generate_playlist" onClick={() => {setReceived(false);setLink("")}}>Generate another playlist</div></>
          )}
    </form>
        </div>
        {link ? <div className='prop'></div> : <Spline scene="https://prod.spline.design/YoQByJBAq8JIoq4S/scene.splinecode" />}
        </div>
      </div>
    </>
  );
